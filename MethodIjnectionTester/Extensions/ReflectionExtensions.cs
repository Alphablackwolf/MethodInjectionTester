...
namespace MethodIjnectionTester.Extensions
{
    /// <summary>
    ...
    public static class ReflectionExtensions
    {
        ...
        public static void CheckPointers(this MethodInfo method)
        {
            unsafe
            {
                Logger.LogInformation("Typical Method Pointer Check:");
                if (IntPtr.Size == 4)
                {
                    var pointer = (uint*) method.MethodHandle.Value.ToPointer() + 2;
                    Logger.LogInformation($"(x86){method.DeclaringType} {method.Name} method pointer:{*pointer}");
                }
                else
                {

                    var pointer = (ulong*) method.MethodHandle.Value.ToPointer() + 1;
                    Logger.LogInformation($"(x64){method.DeclaringType} {method.Name} method pointer:{*pointer}");
                }

                Logger.LogInformation("Virtual method check:");
                var basePointer = (ulong*) method.MethodHandle.Value.ToPointer();
                var index = (int) ((*basePointer >> 32) & 0xFF);
                Logger.LogInformation($"Base Pointer: {*basePointer}, index = {index}");
                Logger.LogInformation($"Method is {(method.IsVirtual ? string.Empty : "not ")}virtual.");
                if (!method.IsVirtual || method.DeclaringType == null) return;
                Logger.LogInformation($"method function pointer: {method.MethodHandle.GetFunctionPointer()}");
                if (IntPtr.Size == 4)
                {

                    var classStart = (uint*) method.DeclaringType.TypeHandle.Value.ToPointer() + 10;
                    classStart = (uint*) *classStart;
                    var methodPointer = classStart + index;
                    Logger.LogInformation($"(x86)class start: {*classStart}, pointer: {*methodPointer}");
                }
                else
                {
                    var classStart = (ulong*) method.DeclaringType.TypeHandle.Value.ToPointer() + 8;
                    classStart = (ulong*) *classStart;
                    var methodPointer = classStart + index;
                    Logger.LogInformation($"(x64)class start: {*classStart}, pointer: {*methodPointer}");
                }
            }
        }
    ...
    public class MethodHook<T> : IMethodHook
    {
        ...
        private static bool Replace(MethodInfo methodToReplace, MethodInfo methodToInject)
        {
            ...
            {
                return ReplaceVirtualInner(methodToReplace, methodToInject);
            }
        ...
        private static bool ReplaceVirtualInner(MethodInfo methodToReplace, MethodInfo methodToInject)
        {
            ...
            unsafe
            {
                ...
                if (IntPtr.Size == 4)
                {
                    ...
                    var injectedMethodPointer = (uint*)methodToInject.MethodHandle.Value.ToPointer() + 2;
                    var baseMethodPointer = (uint*)methodToReplace.DeclaringType.GetMethod(methodToReplace.Name).MethodHandle.Value.ToPointer();
                    Logger.LogInformation($"(x86)Source pointer:{*injectedMethodPointer} | Target pointer: {*targetMethodPointer}, Base method pointer: {*baseMethodPointer}");
                    if (baseMethodPointer == targetMethodPointer)
                    {
                        Logger.LogCriticalWarning("Method replacement failed, target pointer equals the pointer for the base class' method of the same name.");
                        return false;
                    }
                    *targetMethodPointer = *injectedMethodPointer;
                    Logger.LogInformation($"(x86)New Target pointer: {*targetMethodPointer}");
                ...
                else
                {
                    ...
                    var targetMethodPointer = classStart + index;
                    var injectedMethodPointer = (ulong*)methodToInject.MethodHandle.Value.ToPointer() + 1;
                    var baseMethodPointer = (ulong*)methodToReplace.DeclaringType.GetMethod(methodToReplace.Name).MethodHandle.Value.ToPointer();
                    Logger.LogInformation($"(x86)Source pointer:{*injectedMethodPointer} | Target pointer: {*targetMethodPointer}, Base method pointer: {*baseMethodPointer}");
                    if (baseMethodPointer == targetMethodPointer)
                    {
                        Logger.LogCriticalWarning("Method replacement failed, target pointer equals the pointer for the base class' method of the same name.");
